...
<main>
	<article>
		...
		<form:form class="form" action="/user/${user.id}/event/${event.id}/edit" method="POST" modelAttribute="event">
			 <div>
				<form:label class="form-label" path="name">
					<p>Nombre del Evento:</p> 
					<span class="span">(puede ser los nombres de la pareja, esto aparecerá como título en la invitación)</span>
				</form:label>
				<form:input class="form-control mb-4" path="name" type= "text" name ="name"/>
				<form:errors class="alert alert-danger mb-4" path="name"/>
			</div>
			<section>
				<div>
					<form:label class="form-label" path="date">Fecha: </form:label>
					<form:input class="form-control mb-4" path="date" type= "date" name ="date"/>
					<form:errors class="alert alert-danger" path="date"/>
				</div>
				<div>
					<form:label class="form-label" path="start">Inicio: </form:label>
					<form:input class="form-control mb-4" path="start" type= "time" name ="start"/>
					<form:errors class="alert alert-danger" path="start"/>
				</div>
				<div>
			...
			<div>
				<form:label class="form-label" path="photos">
					<p>Plataforma de Fotos</p> 
					<span class="span">(puedes pegar aqui el link a la plataforma de fotos): </span>
				</form:label>
				<form:input class="form-control mb-4" path="photos" type= "text" name ="photos"/>
				<form:errors class="alert alert-danger" path="photos"/>
			</div>
			<div>
    			<form:label class="form-label" path="giftsType">Tipo de regalos: </form:label>
    			<form:select id="input" class="form-select form-select-lg mb-3" aria-label="Large select example" path="giftsType" onchange="updateText()">
			...
			<div id="output" style="display: none;">
    			<form:label class="form-label" path="gifts">
    				...
    				<span class="span">(esto ayudará a tus invitados a darte mejores regalos)</span></form:label>
    			<form:textarea class="form-control mb-4" path="gifts" name="gifts"></form:textarea>
			</div>
			<div class="buttonBox">
				<button class="button3 mt-4">Confirmar</button>
			</div>
		</form:form> 
	</article>
	
	<footer class="buttonBox2">
		<a class="button1 btnBack" href="/user/${user.id}/event/${user.event.id}">
			<ion-icon name="arrow-back-outline"></ion-icon>Volver atrás
		</a>
		<form:form action="/user/${user.id}/event/${event.id}/delete" method="POST">
			<input type="hidden" name="_method" value="DELETE"  />
			<button class="button1 btnBack" onclick="return confirmSubmit()">
			<ion-icon name="trash-outline"></ion-icon>Eliminar Evento</button>
		</form:form>
	</footer>
</main>
